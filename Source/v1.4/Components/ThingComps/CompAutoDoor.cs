        ...
        public override IEnumerable<Gizmo> CompGetGizmosExtra()
        {
            ...
            if (doorRef.Open)
            {
                yield return new Command_Action
                {
                    ...
                    defaultLabel = "BF_AutoDoorClose".Translate(),
                    defaultDesc = "BF_AutoDoorCloseDescription".Translate(),
                    action = delegate ()
                    {
                        ...
                        MoteMaker.ThrowText(doorRef.TrueCenter() + new Vector3(0.5f, 0f, 0.5f), doorRef.Map, "BF_AutoDoorCloseMoteText".Translate(), Color.white, -1f);
                    }
                };
            }
            else
            {
                yield return new Command_Action
                {
                    ...
                    defaultLabel = "BF_AutoDoorOpen".Translate(),
                    defaultDesc = "BF_AutoDoorOpenDescription".Translate(),
                    action = delegate ()
                    {
                        if (!Traverse.Create(doorRef).Field("holdOpenInt").GetValue<bool>())
                        ...
                        MoteMaker.ThrowText(doorRef.TrueCenter() + new Vector3(0.5f, 0f, 0.5f), doorRef.Map, "BF_AutoDoorOpenMoteText".Translate(), Color.white, -1f);
                    }
                };
            }
            yield break;
