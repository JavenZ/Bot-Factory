        ...
        public class DrawOverviewTab_Patch
        {
            ...
            public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> insts, ILGenerator generator)
            {
                ...
                for (int i = 0; i < instructions.Count; i++)
                {
                    ...
                    if (insertionPoint == i)
                    {
                        // If (Utils.IsConsideredMechanical(pawn) && pawn.WorkTypeIsDisabled(BF_WorkTypeDefOf.BF_Mechanic))
                        ...
                        yield return new CodeInstruction(OpCodes.Ldsfld, typeof(BF_WorkTypeDefOf).GetField("BF_Mechanic")); // Load the BF_Mechanic WorkTypeDef onto the stack
                        yield return new CodeInstruction(OpCodes.Call, targetMethod); // Call pawn.WorkTypeIsDisabled(BF_Mechanic);
